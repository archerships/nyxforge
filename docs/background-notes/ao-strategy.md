...
| :--- | :--- | :--- |
| **1. Capital** | Bridging XMR/ZANO for bounty collateral | **Proof of Liquidity:** Emission based on yield/time locked in AO treasury processes. |
...
*   **Bounty Duration:** Supports Social Policy Bonds with maturity dates ranging from 1 year to 200 years.
...
*   **Automation:** This enables truly autonomous bounty expiry checking, NYX token emission, and fee sweeps without human intervention.
...
NyxForge oracles use a **push model**: oracle nodes run as always-on processes that monitor real-world data sources continuously, and *push* a signed attestation to the AO bounty process the moment a goal condition is observed.
...
| **Complexity** | Bounty process must re-fetch and re-evaluate data inside the AO sandbox (requires trusted HTTP adapters). | Data fetch stays off-chain in the oracle node; AO only verifies the signed attestation. |

**Push flow:**

```
Oracle node                   AO Bounty Process
    │                               │
    ├── poll data sources ──────────┤
...
- Bounty expiry enforcement (deadline passed, goal not met → state → Expired)
