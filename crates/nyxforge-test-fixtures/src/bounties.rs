...
use nyxforge_core::bounty::{
    AuctionParams, Bounty, BountyState, GoalMetric, GoalSpec, JudgeSpec, VerificationCriteria,
    ComparisonOp,
};
...
/// First judge key (used in single-judge and quorum tests).
pub const ORACLE_KEY_A: PublicKey = PublicKey([0x22u8; 32]);

/// Second judge key (used in multi-judge quorum tests).
pub const ORACLE_KEY_B: PublicKey = PublicKey([0x33u8; 32]);

/// Third judge key (completes a 3-of-3 quorum).
...
/// Minimal valid GoalSpec — used when tests need a bounty but don't care about the goal.
///
...
// ---------------------------------------------------------------------------
// JudgeSpec constructors
// ---------------------------------------------------------------------------

/// Three-judge quorum spec (A, B, C) — all must agree.
pub fn quorum_3_judge_spec() -> JudgeSpec {
    JudgeSpec {
        quorum: 3,
        judge_keys: vec![ORACLE_KEY_A, ORACLE_KEY_B, ORACLE_KEY_C],
        judge_attest_pks: vec![],
        required_stake: Amount::from_whole(100),
        slash_fraction: Decimal::new(50, 2), // 0.50
    }
}

/// Single-judge spec (A only) — used for tests that need the simplest judge setup.
pub fn single_judge_spec() -> JudgeSpec {
    JudgeSpec {
        quorum: 1,
        judge_keys: vec![ORACLE_KEY_A],
        judge_attest_pks: vec![],
        required_stake: Amount::from_whole(100),
...
/// Relaxed verification for single-judge tests.
pub fn single_judge_verification() -> VerificationCriteria {
    VerificationCriteria {
...
/// Bounty in `Draft` state — all judges accepted, ready for `bounties.issue`.
///
/// Uses [`minimal_goal`] and [`single_judge_spec`] to minimise boilerplate.
/// `return_address` is set to [`ISSUER_KEY`].
pub fn draft_bounty() -> Bounty {
    let goals = vec![minimal_goal()];
    let id = Bounty::compute_id(&goals, &ISSUER_KEY, 42, &ISSUER_KEY);
    Bounty {
        id,
        issuer:            ISSUER_KEY,
        total_supply:      1_000,
        ...
        bounties_remaining:   1_000,
        activated_at_secs: None,
        state:             BountyState::Draft,
        goals,
        judge:            single_judge_spec(),
        verification:      single_judge_verification(),
        created_at_block:  42,
        return_address:    ISSUER_KEY,
    }
}

/// Bounty in `Active` state — collateral locked, notes circulating.
pub fn active_bounty() -> Bounty {
    Bounty { state: BountyState::Active, ..draft_bounty() }
}

/// Bounty in `Proposed` state — published for community review.
pub fn proposed_bounty() -> Bounty {
    Bounty { state: BountyState::Proposed, ..draft_bounty() }
}

/// Bounty in `Expired` state — deadline passed without goal achievement.
pub fn expired_bounty() -> Bounty {
    Bounty { state: BountyState::Expired, ..draft_bounty() }
}

/// Bounty in `Redeemable` state — goal verified, holders may redeem.
pub fn redeemable_bounty() -> Bounty {
    Bounty { state: BountyState::Redeemable, ..draft_bounty() }
}

/// Minimal bounty used in tests that just need a valid [`Bounty`] struct.
///
/// Identical to [`draft_bounty`] — provided as a named alias for readability
/// in tests that don't care about state.
pub fn minimal_bounty() -> Bounty {
    draft_bounty()
}

// ---------------------------------------------------------------------------
// Lifebond keys (separate from general judge pool)
// ---------------------------------------------------------------------------

/// Judge A — geriatric physician, panel lead.
pub const LIFEBOND_ORACLE_KEY_A: PublicKey = PublicKey([0x55u8; 32]);

/// Judge B — second geriatric physician.
pub const LIFEBOND_ORACLE_KEY_B: PublicKey = PublicKey([0x66u8; 32]);

/// Judge C — third geriatric physician.
pub const LIFEBOND_ORACLE_KEY_C: PublicKey = PublicKey([0x77u8; 32]);

/// Judge D — independent actuary (probabilistic verification).
pub const LIFEBOND_ORACLE_KEY_D: PublicKey = PublicKey([0x88u8; 32]);

/// Judge E — government vital records certifier.
...
/// Lifebond term 1: subject is alive.
///
/// `subject.lifebond_001.vital_status >= 1` (1 = alive) by 2125-01-01.
pub fn lifebond_alive_goal() -> GoalSpec {
    GoalSpec {
        title: "Subject Is Alive".into(),
        description: "The bounty subject must be certified alive by vital records authorities.".into(),
        metric: GoalMetric {
...
/// Lifebond term 2: subject is in good health.
///
...
pub fn lifebond_health_goal() -> GoalSpec {
    GoalSpec {
        ...
        description: concat!(
            "The bounty subject must be independently certified in good health ",
...
/// Five-judge medical panel spec for a lifebond (4-of-5 quorum).
///
/// Judges: three geriatric physicians, one actuary, one vital records certifier.
...
pub fn lifebond_judge_spec() -> JudgeSpec {
    JudgeSpec {
        quorum:             4,
        judge_keys:        vec![
...
/// 120-year milestone. DAO override is permitted because the judge composition
/// may need governance adjustment over the multi-decade bounty lifetime.
...
///
/// The issuer locks collateral that pays out to bounty holders only when the
...
///
/// Economics (in DRK):
/// - 10,000 bounties × 100 DRK face value = 1,000,000 DRK collateral locked
/// - Floor price: 0.05 DRK per bounty
///
/// `return_address` is set to [`ISSUER_KEY`] — collateral returns to issuer
/// if the criteria are not both met.
pub fn lifebond() -> Bounty {
    let goals = vec![lifebond_alive_goal(), lifebond_health_goal()];
    let id    = Bounty::compute_id(&goals, &ISSUER_KEY, 1, &ISSUER_KEY);
    Bounty {
        id,
        issuer:            ISSUER_KEY,
        total_supply:      10_000,
...
/// A bounty using the full three-judge quorum and the homelessness goal.
///
/// Use this when the goal content or judge set matters for the test.
/// `return_address` is set to [`ISSUER_KEY`].
pub fn homelessness_bounty() -> Bounty {
    let goals = vec![homelessness_goal()];
    let id = Bounty::compute_id(&goals, &ISSUER_KEY, 100, &ISSUER_KEY);
    Bounty {
        id,
        issuer:            ISSUER_KEY,
        total_supply:      10_000,
...
mod tests {
    ...
    fn state_variants_share_id() {
        assert_eq!(draft_bounty().id, active_bounty().id);
        assert_eq!(draft_bounty().id, expired_bounty().id);
    }
    ...
    fn lifebond_oracle_requires_4_of_5_quorum() {
        let spec = lifebond_judge_spec();
        assert_eq!(spec.quorum, 4);
        assert_eq!(spec.judge_keys.len(), 5);
    }

    #[test]
    fn lifebond_collateral_does_not_overflow() {
        let b = lifebond();
        // 10,000 bounties × 100_000_000 micro-DRK = 1_000_000_000_000 — well within u64
