...
//!
//! ## Push model
//! `JudgeNode::monitor_bounties` spawns a monitoring task per bounty and sends
...
pub struct JudgeConfig {
    ...
    pub bounty_filter: Vec<BountyId>,
...
impl JudgeNode {
    ...
    ///
    /// ```text
    /// bounty_attest_key = blake3(master_sk ‖ bounty_id ‖ "nyxforge::oracle::attest::v1")
    ...
    /// over an encrypted channel.  The bounty holder uses `bounty_attest_key` as
    ...
    pub fn generate_attest_key(&self, bounty_id: BountyId) -> JudgeAttestKey {
        ...
        let bounty_attest_key: [u8; 32] = *h.finalize().as_bytes();

        let judge_attest_pk = judge_attest_pk_from_key(&bounty_attest_key);
        JudgeAttestKey { bounty_attest_key, judge_attest_pk }
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn evaluate_active_bounty_all_goals_met() {
    ...
    #[tokio::test]
    async fn evaluate_active_bounty_goal_not_met() {
    ...
    #[tokio::test]
    async fn evaluate_rejects_non_active_bounty() {
        let node = test_node(vec![mock_source("test.metric", 50.0)]);
    ...
    // --- monitor_bounties() push model ---

    #[tokio::test]
    async fn monitor_bounties_sends_attestation_when_all_goals_met() {
        let node = test_node(vec![mock_source("test.metric", 50.0)]);
        let (tx, mut rx) = mpsc::channel(4);
        let bounty = active_bounty();
        let bounty_id = bounty.id;
        let _handles = node.monitor_bounties(vec![bounty], tx);
    ...
    #[tokio::test]
    async fn monitor_bounties_skips_non_active_bounties() {
        let node = test_node(vec![mock_source("test.metric", 50.0)]);
        let (tx, mut rx) = mpsc::channel(4);
        // Keep a sender clone alive so the channel doesn't close when monitor_bounties
    ...
    #[tokio::test]
    async fn monitor_bounties_does_not_send_when_goal_not_met() {
        // value 150 fails test.metric < 100
        let node = test_node(vec![mock_source("test.metric", 150.0)]);
        let (tx, mut rx) = mpsc::channel(4);
        let bounty = active_bounty();
        let handles = node.monitor_bounties(vec![bounty], tx);
