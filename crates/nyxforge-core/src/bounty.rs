...
/// Parameters governing a Dutch (descending-clock) auction for initial bounty sales.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct AuctionParams {
    /// Ask price at the moment the bounty is activated.
...
/// Unique 32-byte identifier for a bounty series, derived as
/// `blake3(goal_spec_bytes || issuance_timestamp || issuer_pubkey)`.
pub type BountyId = Digest;

/// High-level lifecycle state of a bounty series.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum BountyState {
    ...
    Proposed,
    /// Submitted to the listed judge nodes for acceptance.
    /// Bounty waits here until every judge accepts. Any rejection returns it
    /// here after the issuer revises the judge list or threshold.
    PendingJudgeApproval,
    /// All listed judges have accepted responsibility. Ready for collateral lock.
    Draft,
    /// Collateral locked, bounties circulating on the market.
    Active,
    /// Judge network has confirmed goal achievement; holders may redeem.
    Redeemable,
    /// All bounties in series have been redeemed.
    Settled,
...
pub struct GoalSpec {
    ...
    /// Optional supporting evidence format expected from judges.
...
/// Specification of the judge network trusted to verify the goal.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct JudgeSpec {
    /// Minimum number of judges that must independently attest the result.
    pub quorum: u32,

    /// Ed25519 public keys of approved judge nodes (for slashing / off-chain sig verification).
    pub judge_keys: Vec<PublicKey>,
    ...
    /// Stake (in base token) that each judge must post as collateral.
    pub required_stake: Amount,

    /// Slash fraction (0.0–1.0) if judge posts fraudulent data.
...
pub struct VerificationCriteria {
    /// Number of independent judge attestations required.
    pub attestation_threshold: u32,

    /// How long the challenge window is open after an attestation.
    pub challenge_period_secs: u64,

    /// Whether a DAO governance vote can override judge consensus.
    pub dao_override_allowed: bool,
}

/// A community question or suggestion on a proposed bounty.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BountyComment {
    /// Unique ID: blake3(bounty_id || author || body || created_at).
    pub id: Digest,

    /// The bounty this comment is attached to.
...
/// An judge node's acceptance or rejection of responsibility for a bounty.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct JudgeResponse {
    /// The bounty this response is for.
    pub bounty_id: BountyId,

    /// The judge that responded.
    pub judge_key: PublicKey,

    /// `true` = accepted, `false` = rejected.
    pub accepted: bool,

    /// Required when `accepted = false`; explains why the judge cannot judge
    /// this bounty (ambiguous goal, unsupported data source, etc.).
    pub reason: Option<String>,

    /// Wall-clock time the judge responded.
    pub responded_at: DateTime<Utc>,

    /// Judge signature over (bounty_id || accepted || reason || responded_at).
    /// Stub until real Ed25519 signing is wired in.
    pub signature: Vec<u8>,
}

/// A bounty series — the on-chain record that backs individual bounty notes.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Bounty {
    /// Unique identifier for this bounty series.
    pub id: BountyId,

    /// Compressed public key of the issuer.
    pub issuer: PublicKey,

    /// Total number of bounty units issued.
    pub total_supply: u64,

    /// Amount (in base token) redeemable per bounty unit upon goal achievement.
    pub redemption_value: Amount,

    /// Dutch auction parameters for initial bounty sales.
    pub auction: AuctionParams,

    /// Bounties still available for purchase (decremented on each buy).
    pub bounties_remaining: u64,

    /// Unix timestamp (seconds) when this bounty was activated (issued).
...
impl Bounty {
    /// Compute the bounty's identifier from its canonical fields.
    pub fn compute_id(
        goals: &[GoalSpec],
        issuer: &PublicKey,
        ...
        hasher.update(b"nyxforge::bounty_id");
...
mod tests {
    ...
    fn bond_id_differs_by_title() {
        ...
        let id1 = Bounty::compute_id(&[g1.clone()], &ISSUER_KEY, 0, &RETURN_ADDR);
        let id2 = Bounty::compute_id(&[g2.clone()], &ISSUER_KEY, 0, &RETURN_ADDR);
        assert_ne!(id1, id2);
        // same title again → same id
        g1.title = g2.title.clone();
        assert_eq!(Bounty::compute_id(&[g1], &ISSUER_KEY, 0, &RETURN_ADDR), id2);
    }
    ...
    fn bond_id_differs_by_goals_order() {
        ...
        let id_ab = Bounty::compute_id(&[g1.clone(), g2.clone()], &ISSUER_KEY, 0, &RETURN_ADDR);
        let id_ba = Bounty::compute_id(&[g2, g1], &ISSUER_KEY, 0, &RETURN_ADDR);
